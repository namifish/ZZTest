...
# Funktion zum Hochladen von Daten zu GitHub
def upload_to_github(file_path, repo, github_path):
    with open(file_path, "r") as file:
        content = file.read()
    try:
        # Check if the file exists in the repo
        repo.get_contents(github_path)
        # If it exists, update it
        repo.update_file(github_path, "Updated data", content, repo.get_contents(github_path).sha)
    except:
...
# Wenn der Benutzer authentifiziert ist oder als Gast angemeldet ist
    ...
    # Ansicht "Account"
        ...
        if st.session_state['guest']:
            ...
            # Button zum Hochladen der Daten zur GitHub-Datenbank
            if st.button("Daten auf Datenbank absichern", key="upload_to_github_button"):
                upload_to_github(DB_FILE, repo, GITHUB_PATH + '/zellzaehler.db')
                upload_to_github(LOGIN_FILE, repo, GITHUB_PATH + '/login_hashed_password_list.csv')
